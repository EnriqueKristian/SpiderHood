-- =====================================================================================
-- URGENTE: cierra un hueco de escalación de privilegios. Las pantallas que asignan
-- roles y permisos (/Settings/Roles, /Settings/Roles/Permissions/{id},
-- /Settings/UserRoles) no tenían NINGUNA clave de permiso que las gatee — cualquier
-- usuario autenticado que navegara directo a esas URLs podía asignarse a sí mismo el
-- rol Administrador y cualquier permiso del sistema. "manage_roles" no existía como
-- clave en ninguna parte de la BD.
--
-- Este script crea "manage_roles" y la asigna a Administrador y SysAdmin — el mismo
-- set que ya tienen ambos hoy (57 claves, tras los scripts de Presupuesto y
-- Edificios/Propietarios/Unidades) más esta nueva. No toca Junta ni Residente.
--
...
BEGIN TRY
    ...
    ------------------------------------------------------------------
    IF NOT EXISTS (SELECT 1 FROM Permissions WHERE PermissionKey = 'manage_roles')
    BEGIN
        ...
        VALUES (
            ...
            'Crear/editar roles, asignar permisos a un rol, y asignar un rol a un usuario',
            'settings'
        );
    END

    ------------------------------------------------------------------
    -- 2. Set completo de claves para Administrador / SysAdmin: las 57 que ya tienen
    --    hoy (sin tocarlas) + manage_roles.
    ------------------------------------------------------------------
    ...
    PRINT 'OK: manage_roles creado (si no existía) y asignado a Administrador/SysAdmin.';

    -- Verificación rápida: debería mostrar 58 filas por cada rol.
